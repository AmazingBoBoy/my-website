...
\begin{itemize}
    ...
    \item Algebraic perspective: If $X=X_1\cap X_2$ is disconnected, then $\mathcal{O}(X)\cong \mathcal{O}(X_1)\times \mathcal{O}(X_2)$ (this is just CRT)
...
\begin{itemize}
   ...
   \fbox{we always assume any sheaf $\cF$ of rings is a sheaf of $k$-valued functions}
    ...
    \end{itemize}
\newpage
\section{Morphism of algebraic sets and Ringed Spaces}
\begin{itemize}
    \subsection{Morphism of Algebraic Sets}
    ...
    \item If $U$ is an open set of a ringed space $(X,\cO_X)$, then $(U,\cO_{X\mid U})$ is also a ringed space where $\cO_{X\mid U}$ is the restriction of $\cO_X$ on $U$, which is also a sheaf. 
    ...
    \\
    For any open set $U\subseteq Y$, we define the pullback of a section $\phi \in \cO_Y(U)$ as a function from $f^{-1}(U)$ to $k$ defined by $f^{\#}(\phi)=\phi\circ f$. 
    \\
