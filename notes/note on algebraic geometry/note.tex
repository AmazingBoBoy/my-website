...
\end{itemize}
\newpage
\section{Tools from Commutative Algebra that we need}
Since this is an algebraic geoemtry note, we omit all the proofs in this section. 
\begin{itemize}
    ...
    \begin{example}[Non integral extension]
        ...
        \end{center}
    The geometric meaning of non-integral extension is that there exists a point $B\in \A^1$ such that the preimage of $B$ under this morphism is an infinite set (in this case, the hyperbola approaches the $y$-axis but never touches it, so the preimage of point $B$ on the $y$-axis is empty).
    \end{example}
hard
\end{itemize}
