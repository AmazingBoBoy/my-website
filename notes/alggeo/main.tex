...
\begin{itemize}
    ...
    \item An affine variety $(Y, \SO_Y)$ is a ringed space that is \textbf{isomorphic as ringed space} to $(X,\cO_X)$ for some algebraic set $X$ with Zariski's topology and $\cO_X$ is the sheaf of regular functions on $X$. We actually can have a more explicit description of the regular functions on open sets of $U\subseteq Y$ based on the isomorphism of ringed spaces. 
    \\

    $\phi: U\to k$ is a regular function on $U$ if for any $p\in U$, there exists an open neighborhood $p\in U_p\subseteq U$ and $g,h\in \SO_Y(Y)$ such that, $\phi\reto{U_p}=\frac{g}{h}$ and $h(x)\neq 0$ for all $x\in U_p$.
   ...
   \begin{theorem}
    ...
    \item Now, we can come back to the example above. The map from $\A^1$ to $V(y^2-x^3)\subseteq \A^2$ defined by $t\mapsto (t^2,t^3)$ is not an isomorphism of ringed spaces. It's clearly that it's a bijective morphism of ringed spaces. However, if it's an isomorphism, then $\cO(V(y^2-x^3))\cong \frac{k[x,y]}{(y^2-x^3)}$ must be isomorphic to $\cO(\A^1)=k[t]$ under the map $x\mapsto  t^2$ and $y\mapsto t^3$. But $\cO(V(y^2-x^3))=k[x,y]/(y^2-x^3)$ is not isomorphic to $k[t]$ because $k[x,y]/(y^2-x^3)$ is not a UFD while $k[t]$ is a UFD. Therefore, the map is not an isomorphism of ringed spaces.
    \item (surjectivity is nicer) Let $f: X\to Y$ be a morphism of affine varieties, then 
    \begin{enumerate}
        \item If $f$ is surjective, then $f^*:\cO(Y)\to \cO(X)$ is injective.
        \item If $f^*$ is surjective, then $f$ is injective.
    \end{enumerate}
   \item Subvariety and Ideals: Consider the ideals $I$ of $\SO(X)$ and ${f^*}\inv(I)$ of $\SO(Y)$. They corresponds to $X'\subseteq X$ and $Y'\subseteq Y$. We have $f(X')\subseteq Y'$ and the following commutative diagram:
    % https://q.uiver.app/#q=
\[\begin{tikzcd}
	{\mathcal{O}(X)} && {\mathcal{O}(Y)} && X && Y \\
	\\
	{\frac{\mathcal{O}(X)}{I}=\mathcal{O}(X')} && {\frac{\mathcal{O}(Y)}{{f^*}^{-1}(I)}=\mathcal{O}(Y')} && {X'} && {Y'}
...
\end{proof}
\begin{remark}
    This is not true if $Y$ is not an affine variety. For example, if $Y=\PP^1$, then $\SO_Y(Y)=k$ and there is only one morphism of $k$-algebras from $\SO_Y(Y)$ to $\SO_X(X)$, which is the constant map. However, there are many non-constant morphisms of ringed spaces from $X$ to $\PP^1$ (For example, one can have $[x:y]\mapsto [y:x]$ as a morphism from $\PP^1$ to $\PP^1$ , which is not identity). 
\end{remark}
\begin{example}
    ...
    \begin{proof}
        Assume $\phi: \A^1 \smzero \to \PP^1$ is a morphism. Consider $X_1=\A^1$ and $X_2=\A^1$ as two affine open sets of $\PP^1$. So $U_1=f\phi\inv(X_1)$ and $U_2=\phi\inv(X_2)$ are open set in $\A^1\smzero$. Thus, $U_1$ must be a principle open set of $\A^1$, say $U_f$. Then $\phi\reto{U_f}=\frac{g}{f^n}$ where $g\in k[x]$ and $\gcd(f,g)=1$. 
        \\

        \textbf{Case 1:} If $x\nmid f$, then $\frac{g}{f^n}$ is well-defined at $x=0$. So we can simply extend $\phi$ to a morphism $\wt{\phi}$ from $\A^1$ to $\PP^1$ by sending $0$ to $\frac{g(0)}{f(0)^n}$. $\wt{\phi}$ is a morphism because $\wt{\phi}\reto{U_f\cup \nod }$ is a morphism and $\wt{\phi}\reto{U_2}$ is a morphism., so we can glue then together to get a morphism from $\A^1$ to $\PP^1$.
        \\

        \textbf{Case 2:} If $x\mid f$, then we can consider the open subset $U'$ of $U_f$ where $g\neq 0$ on $U'$. Thus, the image $\phi\reto{U'}$ must be contained in $X_1\smzero =X_1\cap X_2$. So we can consider $\phi\reto{U'}$ as a morphism from $U'$ to $X_2$. In this case, $\phi'\reto{U'}=\frac{1}{\phi\reto{U'}}=\frac{f^n}{g}$ is well-defined at $x=0$. So we can simply extend $\phi'$ to a morphism $\wt{\phi'}$ from $\A^1$ to $\PP^1$ by sending $0$ to $\frac{f(0)^n}{g(0)}=0$.
    \end{proof} 
\end{example}
\begin{example}
    This is not true if we replace $\A^1\smzero$ by $\A^2 \smzero$. Example can be $(x,y)\mapsto [x,y]$ from $\A^2\smzero$ to $\PP^1$. This morphism cannot be extended to a morphism from $\A^2$ to $\PP^1$. 
    \begin{proof}
        Assume it can be extended to a morphism $\wt{\phi}$ from $\A^2$ to $\PP^1$. Consider the $\phi\inv([0:1])$ and $\phi\inv([1:0])$. They are closed subsets of $\A^2$. And $V(y)\smzero \subseteq \phi\inv([0:1])$ and $V(x)\smzero \subseteq \phi\inv([1:0])$. So $\phi\inv([0:1])$ contains $V(y)$ and $\phi\inv([1:0])$ contains $V(x)$. Thus, $0\in \phi\inv([0:1])\cap \phi\inv([1:0])$, which is a contradiction because $[0:1]\neq [1:0]$. Therefore, $\wt{\phi}$ cannot exist.
    \end{proof}
\end{example}
\subsection{Producing Prevarieties}
\subsubsection*{Open subprevariety}
We have discussed this before. This is very easy. Any open subset of a prevariety is a prevariety. The sheaf structure on the open subset is just the restriction of the sheaf structure on the prevariety. And it's clearly covered by finitely many affine open sets. 
\subsubsection*{Closed subprevariety}
If $Y\subseteq X$ is a closed subset of prevariety $X$. The sheaf structure on $Y$ is not just the restriction of the sheaf structure on $X$ because the open sets of $Y$ are not the same as the open sets of $X$. We need to define the sheaf structure on $Y$ in a different way:
\[\SO_Y(U):=\{\phi: U\to k\mid \phi \text{ is locally the restriction of a regular function on } X\}\]
That is, for any $\phi\in \SO_Y(U)$ and any $p\in U$, there exists an open neighborhood $V_p$ of $p$ in $X$ and a $\psi\in |SO_X(V_p)$ such that $\psi\reto{V_P\cap U}=\phi\reto{V_p\cap U}$.
\\

This is clearly a sheaf by the local nature of the definition. But we still need to show that this is a prevariety, that is we can cover $Y$ by finitely many affine open sets. 
\begin{claim}
    Let $Y$ be a closed subset of a prevariety $X$. Let $U$ be an affine open set of $X$, then $(Y\cap U)_1$ as an open subset of $Y$ is isomorphic to $(Y\cap U)_2$ as a closed subset/affine subvariety of open affine variety $U$.
\end{claim}
\begin{proof}
    We underlying function of this morphism is just identity map. We just need show that it's indeed a morphism. For any open subset $L_2\subseteq (Y\cap U)_2$ and any regular function $\phi: L_2 \to k$, it's locally a rational function, which means that for any $p\in L$, there exists $U_p$ an open neighborhood of $p$ in $L$ and $f,g\in O_U(U)$ ($g\neq 0$ on $U_p$) such that $\phi\reto{U_p}=\frac{f}{g}\reto{U_p}$. 
    \\
    For each $U_p$, there exists open subset of $X$, $U_p'\subseteq X$ such that $U_p=L_2\cap U_p'$. 
    \\
    The pull back of $\phi$ is still $\phi$, and we need to verify that it's indeed in $\SO_{Y}(L_1)$: 
    For any $p\in L_1$, we can take open set $V\subseteq X$ such that $V=U_p'\cap U_g$ (this is nice because $V\cap L_1=U_p$). Thus, we can find $\psi=\frac{f}{g}\in \SO_X(V)$ such that $\psi\reto{V\cap L_1}=\frac{f}{g}=\phi\reto{U_V\cap L_1}$. Therefore, the pull back of $\phi$ is in $\SO_Y(L_1)$. 
    \\
    
    Conversely, if we have $\phi\in \SO_Y(L_1)$, then for any $p\in L_1$, there exists an open neighborhood $V_p$ of $p$ in $X$ and $\psi_p\in \SO_X(V_p)$ such that $\psi_p\reto{V_p\cap L_1}=\phi\reto{V_p\cap L_1}$. We may further assume $V_p\subseteq U$. So we can assume $\psi_p\in \SO_X(V_p)=\SO_U(V_p)$. So $\psi_p$ is locally a rational function, which means that for any $p\in L_2$, there exists $U_p=V_p\cap L_2$ an open neighborhood of $p$ in $L_2$ and $f,g\in O_U(V_p)$ ($g\neq 0$ on $U_p$) such that $\phi\reto{U_P}=\psi_p\reto{U_p}=\frac{f}{g}\reto{U_p}$. Thus, $\phi\reto{U_p}=\frac{f}{g}\reto{U_p}$, which is a regular function on $L_2$. We're done. 
\end{proof}
\begin{proposition}
    Propositions of closed subprevariety $Y\subseteq X$:
    \begin{enumerate}
        \item $i:Y\to X$ is a morphism of ringed spaces.
        \item If morphism of ringed space $f:Z\to X$ has image contained in $Y$, then $f$ can be considered as a morphism of ringed spaces from $Z$ to $Y$. (regular functions on $Y$ is )
    \end{enumerate}
\end{proposition}
\subsubsection*{Product of Prevarieties}
Let $X$ and $Y$ be two prevarieties. we can define the product of $X$ adn $Y$ by gluing the products of affine open sets in $X$ and $Y$. More precisely, let $\{X_i\}_{i=1}^m$ and $\{Y_j\}_{j=1}^n$ be finite affine open covers of $X$ and $Y$ respectively. Then we can glue the affine varieties $X_i\times Y_j$ together to get a prevariety, which is defined to be the product of $X$ and $Y$. We denote it by $X\times Y$. This product is independent of the choice of affine open covers of $X$ and $Y$ because we can verify its universal property of product in the category of prevarieties. We omit the details here.

\subsection{Algebraic Varieties}
As we have seen in the example of affine line with double origin, prevarieties can be somewhat pathological. So we need to impose some extra conditions to get rid of those pathologies. similar to our restriction on manifolds, we need to impose some extra conditions on prevarieties to get algebraic varieties. The condition is separatedness, which is the analogue of Hausdorff condition in topology. 
\begin{definition}
    A prevariety $X$ is called a variety if the diagonal $\Delta_X=\{(x,x)\in X\times X\mid x\in X\}$ is a closed subset of $X\times X$ where $X\times X$ is the product in category of prevarieties.
\end{definition}
\begin{example}
    All affine varieties are varieties because the diagonal is closed in the product of affine varieties.
\end{example}
\begin{example}
    The open and closed subprevarieties of a variety are also varieties. 
\end{example}
\begin{example}
    The projective line $\PP^1$ is a variety.
\end{example}
\begin{example}
    The affine line $\ell$ with double origin is not a variety. $\Delta_\ell$ doesn't contain the point $(0_1, 0_2)$ so it's not closed in $\ell_2\times \ell_1$.
\end{example}
\begin{proposition}
    Assume $f,g$ are morphisms of ringed spaces $f:X\to Y$ where $Y$ are varieties, then
    \begin{enumerate}
        \item The graph $\Gamma_f=\{(x,f(x))\in X\times Y\mid x\in X\}$ is a closed subset of $X\times Y$.
...
\begin{proposition}
    ...
    \begin{enumerate}
        \item $X\times Y$ is a variety if $X$ and $Y$ are varieties.
        \item The intersection of two affine open sets in a variety is an affine open set. 
    \end{enumerate}
\end{proposition}
\newpage
\section{Projective Varieties: Topology}
\subsection{Definition}
\begin{definition}
    The projective space $\PP^n$ is defined as the set of lines through the origin in $\AA^{n+1}$. $\PP^n=\AA^{n+1}\smzero / \sim$ where $\sim$ is the equivalence relation defined as $v\sim w$ if and only if $v=\lambda w$ for some $\lambda \in k\smzero$. We denote the equivalence class of $v=(a_0, a_1, \ldots, a_n)$ by $[a_0:a_1:\cdots :a_n]$.
\end{definition}
Assume $k=\CC$. We can equip the classic topology on $\PP^n$ by considering $\PP^n$ as the quotient of $\CC^{n+1}\smzero$. Thne, $\PP^n$ is a homoemorphic image of $S^n=\{(x_0:x_1:\cdots:x_n)\mid \sum x_i^2=1\}$. 
\[ S^n\injto\CC^{n+1}\smzero \surjto \PP^n \]
So $\PP^n$ is compact in classic topology. 
\subsection{Projective Varieties}
Given a polynomial $f\in k[x_0, x_1, \ldots, x_n]$, we can consider the zero set of $f$ in $\PP^n$. However, this is not well-defined because if $v\sim w$, then $f(v)=0$ does not necessarily imply $f(w)=0$. For example, if $f=x_0+1$, then $f(-1,0,\ldots, 0)=0$ but $f(-2,0,\ldots, 0)\neq 0$. So we need to restrict ourselves to homogeneous polynomials.
\begin{definition}
    Let $S$ be a set of homogeneous polynomials in $k[x_0, x_1, \ldots, x_n]$. The projective variety defined by $S$ is the set of points in $\PP^n$ where all the polynomials in $S$ vanish. We denote it by $V_p(S)$.
    \\

    Notice that $V_p(S)=V_p(\langle S\rangle)$ where $\langle S\rangle$ is the ideal generated by $S$. 
\end{definition}
$\langle S \rangle$ is not just an ideal, but a homogeneous ideal because $S$ consists of homogeneous polynomials. 
\begin{proposition}
    Let $I$ be an ideal in a graded ring $R$. 
    \begin{enumerate}
        \item $I$ is a homogeneous ideal if and only if $I$ is generated by homogeneous polynomials if and only if for any $f\in I$, all the homogeneous components of $f$ are in $I$.
        \item If $J_1, J_2$ are two homogeneous ideals, then $J_1+J_2$, $J_1\cap J_2$, $J_1J_2$ and $\sqrt{J_1}$ are all homogeneous ideals.
        \item If $I$ is a homogeneous ideal,$R/I$ is a graded ring with the grading defined as $(R/I)_d=R_d/(I\cap R_d)$. 
    \end{enumerate}
\end{proposition}
There is a nice correspondence between affine variety and projective variety. 
\begin{definition}
    Let $\pi: \AA^{n+1}\smzero \to \PP^n$ be the quotient map. 
    \begin{enumerate}
        \item An affine variety $X\subseteq \AA^{n+1}$ is called a cone if $0\in X$ and $\lambda x\in X$ for any $x\in X$ and $\lambda \in k$. In other words, $X$ is the union of lines through the origin.
        \item For a cone $X$, we define $\PP(X)=\pi(X\smzero)$ to be the projectivization of $X$. 
        \item For a projective variety $Y\subseteq \PP^n$, we define $C(Y)=\nod\cup \pi\inv(Y)$ to be the cone over $Y$.
    \end{enumerate}
\end{definition}


\begin{theorem}
    There is a bijection between cones in $\AA^{n+1}$ and projective varieties in $\PP^n$. 
    % https://q.uiver.app/#q=
\[\begin{tikzcd}
	{\{\text{Cones in } \AA^{n+1}\}} && {\{\text{projective varieties in  } \PP^{n}\}} \\
	X && {\PP(X)} \\
	{C(Y)} && Y
	\arrow["{1:1}", tail reversed, from=1-1, to=1-3]
...
\end{theorem}
\begin{proof}
    First, we show that if $X$ is an affine cone, then $\PP(X)$ is a projective variety. Since $X$ is an affien variety, $X=V_a(I)$ for some ideal $I$. We claim that $I$ is homogeneous. For any $f\in I$, we can write $f=f_0+f_1+\cdots +f_d$ where $f_i$ is the homogeneous component of $f$ of degree $i$. Since $X$ is a cone, for any $\lambda \in k$ and any $x\in X$, $\lambda x\in X$. So $0=f(\lambda x)=\sum_{i=0}^d \lambda^if_i(x)$ for any $\lambda \in k$ and any $x\in X$. Since $\sum t^i f_i(x)$ is a polynomial in $t$ that vanishes for infinitely many values of $t$, it must be the zero polynomial. So $f_i(x)=0$ for any $i$ and any $x\in X$. Thus, $f_i\in I$ for any $i$. Therefore, $I$ is a homogeneous ideal. Then, it's clear that $\PP(X)=V_p(I)$ is a projective variety.
    \\

    Conversely, if $Y$ is a projective variety $V_p(J)$ for some homogeneous ideal $J$, then $C(Y)=\nod\cup \pi\inv(Y)=V_a(J)$ is an affine variety. It's a cone because $0\in C(Y)$ and $\lambda x\in C(Y)$ for any $x\in C(Y)$ and $\lambda \in k$.
    \\

    We can easily check that $C(-)$ and $\PP(-)$ are inverse of each other. So there is a bijection between cones in $\AA^{n+1}$ and projective varieties in $\PP^n$.
\end{proof}
\begin{definition}
    For a subset $X$ of $\PP^n$, we define the homogeneous ideal of $X$ to be $I_p(X)=\langle f\in k[x_0, x_1, \ldots, x_n]\mid f \text{ is homogeneous and } f(x)=0 \text{ for any } x\in X\rangle$.
\end{definition}
\begin{theorem}[Porjective Nullstellensatz]
    There is a one to one inclusion-reversing correspondence between projective varieties in $\PP^n$ and homogeneous radical ideals in $k[x_0, x_1, \ldots, x_n]$ that do not contain the irrelevant ideal $\langle x_0, x_1, \ldots, x_n\rangle$. 

    % https://q.uiver.app/#q=WzAsMixbMCwwLCJcXHRleHR7UHJvamVjdGl2ZSB2YXJpZXJ0aWVzIGluIH0gXFxQUF5uIl0sWzIsMCwiXFx0ZXh0e05vbi1pcnJlbGV2YW50IGhvbW9nZW5ldXMgcmFkaWNhbCByYWRpY2FsIGluIH0gS1t4XzAsIFxcbGRvdHMsIHhfbl0iXSxbMCwxLCIxOjEiLDAseyJzdHlsZSI6eyJ0YWlsIjp7Im5hbWUiOiJhcnJvd2hlYWQifX19XV0=
\[\begin{tikzcd}
	{\text{Projective varierties in } \PP^n} && {\text{Homogeneus radical } J\neq I_0}
	\arrow["{1:1}", tail reversed, from=1-1, to=1-3]
\end{tikzcd}\]
\end{theorem}
Follow from the correspondence between cones in $\AA^{n+1}$ and projective varieties in $\PP^n$ and the Nullstellensatz for affine varieties. 
\\

The small bug happen at $\emptyset$ corresponds to both irrelevant ideal and the whole ring. 
\begin{corollary}
    $V_p(J_1)\cap V_p(J_2)=V_p(J_1+J_2)$ and $V_p(J_1)\cup V_p(J_2)=V_p(J_1J_2)$ for any homogeneous ideals $J_1$ and $J_2$.
    \\

    $I_p(X\cap Y)=\sqrt{I_p(X)+I_p(Y)}$ unless $X\cap Y=\emptyset$ and $I_p(X\cup Y)=I_p(X)\cap I_p(Y)$ for any projective varieties $X$ and $Y$ of $\PP^n$.
\end{corollary}
From corollary above, we can define a topology on $\PP^n$ by taking projective varieties as closed sets. This is called Zariski's topology on $\PP^n$. 
\\

We can also give the relative version of the Nullstellensatz for projective varieties. First, we define the coordinate ring of a projective variety $X$ to be $S(X)=k[x_0, x_1, \ldots, x_n]/I_p(X)$. Then we have the following theorem:
\begin{theorem}
    There is a one to one inclusion-reversing correspondence between projective subvarieties of a projective variety $X$ and homogeneous radical ideals in $S(X)$ $\neq$ the irrelevant ideal. 
\end{theorem}
\begin{lemma}
    As a analogy of affine case, we can see that homogeneous prime ideals of $S(X)$ $\neq$ the irrelevant ideal correspond to irreducible projective subvarieties of $X$.
    \\

    This is not true for point to maximal ideal. Let $p=[p_0:p_1:\cdots:p_n]$ be a point and assume $p_0=1$. Then $I_p(\{p\})=\langle x_1-x_0p_1, \ldots, x_n-x_0p_n \rangle$ 
\end{lemma}
\begin{remark}
    Notice that $V_p(f)=V_p(x_0^df, x_1^df, \ldots, x_n^df)$ for any homogeneous polynomial $f$ of degree $d$. So we can always assume the ideal defining a projective variety is generated by homogeneous polynomials of the same degree.
\end{remark}
\subsection{Homogenization and dehomogenization}
Let $f\in k[x_0, x_1, \ldots, x_n]$ be a homogeneous polynomial of degree $d$. We define $f^i=f(1, x_1, \ldots, x_n)$ to be the dehomogenization of $f$ and for any $g\in k[x_1, \ldots, x_n]$ of degree $d$, we define $g^h=x_0^dg(\frac{x_1}{x_0}, \ldots, \frac{x_n}{x_0})$ to be the homogenization of $g$. 
\begin{proposition}
    $V_p(J)\cap \AA^n=V_a(J^i)$ and $V_a(I)=V_p(I^h)\cap \AA^n$ for any homogeneous ideal $J$ in $k[x_0, x_1, \ldots, x_n]$ and any ideal $I$ in $k[x_1, \ldots, x_n]$. 
    \\

    With this proposition, we can see that the map from $\AA^n$ to $U_i:=\{[x_0:x_1:\cdots:x_n]\in \PP^n\mid x_i\neq 0\}\subseteq \PP^n$ defined by $(a_1, \ldots, a_n)\mapsto [1:a_1:\cdots:a_n]$ is a homeomorphism between $\AA^n$ and $U_i$ with respect to Zariski's topology. So we can identify $\AA^n$ with $U_i$ and consider $\AA^n$ as an open subset of $\PP^n$.
\end{proposition}
\begin{corollary}
    $\PP^n$ is irreducible with dimension $n$. 
\end{corollary}
\begin{proof}
    $\PP^n$ is covered by $n+1$ affine open sets $U_i$ which are all irreducible with dimension $n$ with nonempty intersection. So $\PP^n$ is irreducible with dimension $n$.
\end{proof}
\begin{theorem}
    Let $J$ be an ideal of $k[x_0, x_1, \ldots, x_n]$ and $X=V_a(J)$ be the affine variety defined by $J$. Let $\ol{X}$ be the closure of $X$ in $\PP^n$. Then $\ol{X}=V_p(J^h)$.
\end{theorem}
\begin{proof}
    It suffcies to show that for any $V_p(I)$ containing $X$, $V_p(I)$ also contains $V_p(J^h)$. Thus, it suffices to show that $\sqrt{I}\subseteq \sqrt{J^h}$. For any homogeneous polynomial $f\in \sqrt{I}$, $f^n\in I$ for some $n$. So $f$ vanishes on $X$. That is $f^i$ vanishes on $X$. So $f^i\in \sqrt{J}$, so $(f^n)^i\in J$. So $f^n\in J^h$. Thus, $f\in \sqrt{J^h}$. Therefore, $\sqrt{I}\subseteq \sqrt{J^h}$ and we're done.
\end{proof}
So it's curcial to homogenize the ideal. But in general, homogenized the generators of $I$ doesn't necessarily generate $I^h$. For example, let $I=(x_1, x_2-x_1^2)$. $I^h=(x_1, x_2)\neq (x_1, x_0x_2-x_1^2)$. The main reason is that $(f+g)^h$ is not necessarily equal to $f^h+g^h$. However, if we have a Grobner basis of $I$, then we can homogenize the Grobner basis to get a Grobner basis of $I^h$.
\begin{theorem}
    Let $G$ be a graded Grobner basis of $I$ with respect to some monomial order (prioritize higher degrees). Then $G^h=\{g^h\mid g\in G\}$ is a Grobner basis of $I^h$ with respect to the order with prioritizing $x_0$ the least. 
    (Given $x^ax_0^d$ and $x^bx_0^t$, we say $x^ax_0^d$ is greater than $x^bx_0^t$ if the total degree $x^ax_0^d$ is greater than the total degree $x^bx_0^t$ or if $a>b$ with respect to the original monomial order)
\end{theorem}
\begin{proof}
    \begin{lemma}
        $Lt(f)=Lt(f^i)$ for any homogeneous polynomial$f\in k[x_0, x_1, \ldots, x_n]$ for some $d$. And $Lt(g)=Lt(g^h)$ for any $g\in k[x_1, \ldots, x_n]$. 

    \end{lemma}
    For any $f\in I^h$, we want to find its leading term. We may assume $f$ is homogeneous since the leading term of a non-homogeneous polynomial is the leading term of its homogeneous part with the higest degree (our monomial order is graded). We can write $f=\sum p_jg_j^h$ for some $p_j\in k[x_0, x_1, \ldots, x_n]$ and $g_j\in G$. $f^i=\sum p_j^i(g_j^h)^i$. Notice that $(g_j^h)^i=g_j$. So $f^i\in I$. So $Lt(f)=Lt(f^i)$ is generated by $Lt(g_j)=Lt(g_j^h)$. Therefore, $G^h$ is a Grobner basis of $I^h$.
\end{proof}
\begin{remark}
    The monomial order in the theorem above must be graded. Lexicographic order doesn't work. For example, let $I=(x-y^2, x^2-y^3)$. $G=\{x-y^2, y^4-y^3\}$. $G^h=\{xz-y^2, y^4-y^3z\}$ is not a Grobner basis of $I^h$: $xy^2-y^3\in I^h$ but $Lt(xy^2-y^3)=xy^2$ is not generated by $Lt(g^h)$'s.
\end{remark}
\begin{corollary}
    If $J$ is principal, then $J^h$ is also principal.
\end{corollary}
\begin{corollary}
    any hypersurjace in $\PP^n$ can be defined by a single homogeneous polynomial.
\end{corollary}
\begin{proof}
    Let $X$ be a hypersurface in $\PP^n$. WLOG, $X$ does not contain $V_p(x_0)$. Let $Y=X\cap \AA^n$. 
    \begin{claim}
    $\ol{Y}=X$.
    \begin{subproof}
        We can decompose $X$ into irreducible components $X=X_1\cup \cdots \cup X_m$. Then $Y=Y_1\cup \cdots \cup Y_m$ where $Y_i=X_i\cap \AA^n$. Notice that $\ol{Y}=\ol{Y_1\cup \cdots \cup Y_m}=\ol{Y_1}\cup \cdots \cup \ol{Y_m}$. So it suffices to show that $\ol{Y_i}=X_i$ for any $i$. $Y_i=X_i\cap \AA^n=X_i\sm V_p(x_0)$ is a nonempty open subset of $X_i$. So $\ol{Y_i}=X_i$ because $X_i$ is irreducible. Therefore, $\ol{Y}=X$.
    \end{subproof}
    \end{claim}
    witht the claim, we conclude that $\dim(Y)=\dim(X)=n-1$. So $Y$ is a hypersurface in $\AA^n$. So $Y=V_a(f)$ for some $f\in k[x_1, \ldots, x_n]$. So $\ol{Y}=V_p(f^h)$. Thus, $X=V_p(f^h)$ is defined by a single homogeneous polynomial.
\end{proof}
\end{document}