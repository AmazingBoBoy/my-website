...
\begin{enumerate}
\item Set up Wolfram. 
\item Weddernburn Little Theorem: all finite division rings are fields. The proof by Ernst Witt is increbibly clever. It uses class formula of group and the cyclotomic polynomial.
\item A Riemann integrable function has integral $0$ iff it's zero almost everywhere (set of roots is a zero set).
\item Antiderivative Theorem: The antiderivative of a Riemann integrable function, if it exists, differs from the indefinite integral by a constant. This can be proved using Meaan Value Theorem. 
\end{enumerate}
\section*{Nov. 5, 2025}
\begin{enumerate}
